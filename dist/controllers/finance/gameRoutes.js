...
const buildGameSessionId = (date, time) => {
    if (!date || !time)
        return null;
    const dateOnly = String(date).split('T')[0];
    return `${dateOnly}_${time}`;
};
const emitFinanceUpdate = (teamId, payload) => {
    (0, socket_1.emitToTeam)(teamId, 'financeSummaryUpdated', Object.assign(Object.assign({ team_id: teamId }, payload), { at: new Date().toISOString() }));
};
function registerGameRoutes(router) {
    ...
    router.put('/game-sessions/:game_session_id', verifyToken_1.verifyToken, authz_1.requireManager, (req, res) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            const currentGameRes = yield client.query('SELECT game_id, date, base_cost, notes, game_session_id FROM games WHERE game_session_id = $1 AND team_id = $2 FOR UPDATE', [game_session_id, team_id]);
            ...
            if (targetSessionId !== currentGame.game_session_id) {
                const conflictRes = yield client.query('SELECT game_id FROM games WHERE game_session_id = $1 AND game_id <> $2', [targetSessionId, currentGame.game_id]);
            ...
            const updateGameRes = yield client.query(`
          UPDATE games
          SET
    ...
    router.post('/game-sessions/:game_session_id/players', verifyToken_1.verifyToken, authz_1.requireManager, (req, res) => __awaiter(this, void 0, void 0, function* () {
        var _a, _b;
        const { game_session_id } = req.params;
        const team_id = (0, authz_1.getTeamId)(req);
        ...
        try {
            yield client.query('BEGIN');
            const gameRes = yield client.query('SELECT game_id, base_cost FROM games WHERE game_session_id = $1 AND team_id = $2 FOR UPDATE', [game_session_id, team_id]);
            if (gameRes.rows.length === 0) {
                yield client.query('ROLLBACK');
                res.status(404).json({ success: false, message: 'Game session not found' });
                return;
            }
            const userRes = yield client.query('SELECT username, custom_game_cost FROM users WHERE username = $1 AND team_id = $2', [username, team_id]);
            ...
            const existingRes = yield client.query('SELECT attendance_id FROM game_attendance WHERE game_id = $1 AND username = $2', [game.game_id, username]);
            ...
            const defaultCost = (_b = (_a = userRes.rows[0].custom_game_cost) !== null && _a !== void 0 ? _a : game.base_cost) !== null && _b !== void 0 ? _b : 0;
            const appliedCost = appliedCostInput !== null && appliedCostInput !== void 0 ? appliedCostInput : defaultCost;
            const insertRes = yield client.query(`
          INSERT INTO game_attendance (game_id, username, applied_cost, adjustment_note)
          VALUES ($1, $2, $3, $4)
    ...
    router.put('/game-attendance/:attendance_id', verifyToken_1.verifyToken, authz_1.requireManager, (req, res) => __awaiter(this, void 0, void 0, function* () {
        const { attendance_id } = req.params;
        const team_id = (0, authz_1.getTeamId)(req);
        ...
        try {
            const updateRes = yield userModel_1.default.query(`
